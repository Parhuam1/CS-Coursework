...
{
	return (whole == other.whole && part == other.part);
}

bool Currency::operator==(int other)
{
	return false;
}
...
std::istream& operator >> (std::istream& input_1, Currency &obj_1)
{
	...
	// valid input checking including non-digit characters
	while (true) {
		if (input_1 >> temp && temp > 0) {
			// valid number
			break;
		}
		else {
			// not a valid number
			std::cout << "Please enter a non-negative numerical value: ";
			input_1.clear();
			while (input_1.get() != '\n'); // empty loop
		}
	}

	//old error checking -- did not check for non-digit
	//while (temp < 0)
	//{
